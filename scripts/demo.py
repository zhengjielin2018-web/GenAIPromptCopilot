...
2. **複合屬性用複合 tag 完整表達，但僅限使用者真的描述了複合屬性時**（雙色髮、半邊、漸層、混色這類）：\
遇到時要寫成像 `split-color hair, two-tone hair, purple hair, pink hair` 這種能讓生圖模型理解結構的組合，\
不可被候選片段裡的單色詞（如 `pink hair`）取代或吃掉一半；知識庫沒有的詞由你自己翻譯，不因為片段裡沒有就省略。\
**上面的範例只是示範格式，範例中的詞本身不可原封抄進提示詞**：使用者只描述單一髮色（或其他單一屬性）時，\
就只寫那一個詞，不要多寫。
