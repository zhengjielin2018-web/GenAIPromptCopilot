...
export interface DashboardChip { id: string; label: string; hint: string; state: FacetState; tags: string | null }
...
export function dashboardRows(catalog: FacetCatalog, profile: string | null, facetStates: Record<string, FacetState>, highlighted: string[], facetTags: Record<string, string> = {}): DashboardRow[] {
  ...
  const rows: DashboardRow[] = catalog.dimensions.map(d => {
    ...
    const chips: DashboardChip[] = ids.map(id => {
      const f = byId.get(id)
      return { id, label: f?.label ?? id, hint: f?.hint ?? '', state: facetStates[id] ?? 'missing', tags: facetTags[id] ?? null }
    })
  ...
  for (const [id, state] of Object.entries(facetStates)) {
    ...
    if (row) row.chips.push({ id, label: f?.label ?? id, hint: f?.hint ?? '', state, tags: facetTags[id] ?? null })
  }
  return rows
}
