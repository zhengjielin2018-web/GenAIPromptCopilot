...
`recommendations` 事件與其他事件一樣存 transcript；`adopt` 送出當下 persist 的 `draft` 存伺服器組的那句（重載時回到輸入框的行為與一般訊息一致）。
