...
**修正**（分支 `fix/ask-all-missing`，commit hash 待 merge 後補）：system.md 第 1 條改為 `SetProfile` → `SetFacetStates` → `SearchPresets`；同時把追問政策反轉為問滿 missing 維度（使用者 2026-09-25 實測回饋「描述缺很多面向，但追問很少」，見 #5 的 eval #18）。`SetFacetStates` 與 `AskUser` 的工具描述、主規格 §4.2 與 §15、批次設計 §3.4 同步。主規格 §9「grounded 由伺服器算」原則不變，只是讓伺服器有資料可算。
