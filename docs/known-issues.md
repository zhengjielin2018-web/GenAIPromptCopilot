...
| 9 | 整套組合推薦的已知限制：錨靠模型翻譯、比對只看字面、採用輪失敗後重試是純文字 | 限制 | 低 |
...
- **錨的 SQL 比對只做小寫與底線換空白**：`RecommendAsync` 對資料庫的 tag 只做 `replace(lower(tag), '_', ' ')`，沒有 `TagAttribution.Normalize` 剝 `:數字` 權重、外層括號、連續空白那幾步；`facet_tags` 保留原始 SD 語法（如 `(sandals:1.2)`）時錨會漏配。漏到不足 2 筆就退回無錨，卡片照樣有推薦，所以不容易被發現。SQL 的比對規則比 C# `Normalize` 粗。因此當 `facet_tags` 保留 SD 語法（如 `(sandals:1.2)`）時，被別的錨撈進來的候選可能在回報的 `anchorTags` 裡多列一個 SQL 沒真正比中的錨。
